...
public class WingSolver extends AbstractSolver {
    ...
    private int[] biCells = new int[Sudoku.LENGTH];
    /** The indices of all trivalue cells in the current sudoku (for XYZ-Wing) */
    private int[] triCells = new int[Sudoku.LENGTH];
    ...
    private SolutionStep getWing(boolean xyz, boolean onlyOne) {
        ...
        for (int i = 0; i < endIndex; i++) {
            for (int j = xyz ? 0 : i + 1; j < biValueCount; j++) {
                ...
                if (Sudoku.ANZ_VALUES[sudoku.getCell(biTri[i]) | sudoku.getCell(biCells[j])] != 3) {
                    // cant become a wing
                    continue;
                }
                for (int k = j + 1; k < biValueCount; k++) {
                    ...
                    if (Sudoku.ANZ_VALUES[cell1 | cell2 | cell3] != 3) {
                    ...
                    for (int tries = 0; tries < maxTries; tries++) {
                        ...
                        if (! Sudoku.buddies[index1].contains(index2) || ! Sudoku.buddies[index1].contains(index3)) {
                        ...
                        if (Sudoku.ANZ_VALUES[cell] != 1) {
                            // no wing, sorry
                            continue;
                        }
                        int candZ = Sudoku.CAND_FROM_MASK[cell];
                        // are there candidates that can see the pincers?
                        elimSet.setAnd(Sudoku.buddies[index2], Sudoku.buddies[index3]);
                        elimSet.and(finder.getCandidates()[candZ]);
                        if (xyz) {
                            // the pivot as well
                            elimSet.and(Sudoku.buddies[index1]);
                        }
                        if (elimSet.isEmpty()) {
    ...
    private SolutionStep getWWing(boolean onlyOne) {
        for (int i = 0; i < sudoku.getCells().length; i++) {
            ...
            preCalcSet1.setAnd(Sudoku.buddies[i], finder.getCandidates()[cand1]);
            preCalcSet2.setAnd(Sudoku.buddies[i], finder.getCandidates()[cand2]);
            ...
            for (int j = i + 1; j < sudoku.getCells().length; j++) {
                ...
                elimSet.setAnd(preCalcSet1, Sudoku.buddies[j]);
                if (! elimSet.isEmpty()) {
                ...
                elimSet.setAnd(preCalcSet2, Sudoku.buddies[j]);
                if (! elimSet.isEmpty()) {
                ...
                for (int i = 0; i < indices.length; i++) {
                    ...
                    if (aktIndex != index1 && aktIndex != index2 && sudoku.isCandidate(aktIndex, cand2)) {
                        ...
                        if (Sudoku.buddies[aktIndex].contains(index1)) {
                            sees1 = true;
                            wIndex1 = aktIndex;
                        } else if (Sudoku.buddies[aktIndex].contains(index2)) {
                            sees2 = true;
                            wIndex2 = aktIndex;
                        }
                    }
