...
public class SingleDigitPatternSolver extends AbstractSolver {
    ...
    private int[][] only2Indices = new int[2 * Sudoku.UNITS][2];
    ...
    private SolutionStep findEmptyRectanglesForCandidate(int cand, boolean onlyOne) {
        ...
        for (int i = 0; i < Sudoku.BLOCK_TEMPLATES.length; i++) {
            ...
            blockCands.and(Sudoku.BLOCK_TEMPLATES[i]);
            ...
            for (int j = 0; j < erSets[i].length; j++) {
                ...
                tmpSet.setAnd(blockCands, Sudoku.LINE_TEMPLATES[erLines[i][j]]);
                if (tmpSet.size() >= 2) {
                    notEnoughCandidates = false;
                }
                tmpSet.andNot(Sudoku.COL_TEMPLATES[erCols[i][j]]);
                if (tmpSet.isEmpty()) {
                    // not valid!
                    continue;
                }
                ...
                tmpSet.setAnd(blockCands, Sudoku.COL_TEMPLATES[erCols[i][j]]);
                if (tmpSet.size() >= 2) {
                    notEnoughCandidates = false;
                }
                tmpSet.andNot(Sudoku.LINE_TEMPLATES[erLines[i][j]]);
                if (tmpSet.isEmpty()) {
                    // not valid!
                    continue;
                }
                ...
                SolutionStep step = checkEmptyRectangle(cand, i, blockCands, Sudoku.LINES[erLine], Sudoku.LINE_TEMPLATES,
                        Sudoku.COL_TEMPLATES, erCol, false, onlyOne);
                if (onlyOne && step != null && ! Options.getInstance().isAllowDualsAndSiamese()) {
                    return step;
                }
                step = checkEmptyRectangle(cand, i, blockCands, Sudoku.COLS[erCol], Sudoku.COL_TEMPLATES,
                        Sudoku.LINE_TEMPLATES, erLine, true, onlyOne);
                if (onlyOne && step != null && ! Options.getInstance().isAllowDualsAndSiamese()) {
                    return step;
                }
            }
     ...
     * @param LINE_TEMPLATES Sudoku.LINE_TEMPLATES/Sudoku.COL_TEMPLATES
     * @param COL_TEMPLATES Sudoku.COL_TEMPLATES/Sudoku.LINE_TEMPLATES
                ...
                if (tmpSet.size() == 2) {
                    ...
                    for (int j = 0; j < tmpSet.size(); j++) {
                        ...
                        if (colDel == firstCol) {
                            ...
                            globalStep.setEntity(Sudoku.BLOCK);
        ...
        for (int cand = 1; cand <= 9; cand++) {
            ...
            for (int constr = cStart; constr < cEnd; constr++) {
                if (free[constr][cand] == 2) {
                    ...
                    int[] indices = Sudoku.ALL_UNITS[constr];
                    int candIndex = 0;
                    for (int i = 0; i < indices.length; i++) {
            ...
            for (int i = 0; i < constrCount; i++) {
                for (int j = i + 1; j < constrCount; j++) {
                    ...
                    if (lines) {
                        ...
                        if (Sudoku.getCol(only2Indices[i][0]) == Sudoku.getCol(only2Indices[j][0])) {
                            found = true;
                        }
                        if (! found && Sudoku.getCol(only2Indices[i][1]) == Sudoku.getCol(only2Indices[j][1])) {
                            found = true;
                            otherIndex = 0;
                        }
                    } else {
                        // must be in the same line
                        if (Sudoku.getLine(only2Indices[i][0]) == Sudoku.getLine(only2Indices[j][0])) {
                            found = true;
                        }
                        if (! found && Sudoku.getLine(only2Indices[i][1]) == Sudoku.getLine(only2Indices[j][1])) {
                            found = true;
                            otherIndex = 0;
                        }
                    }
                    ...
                    if (lines && Sudoku.getCol(only2Indices[i][otherIndex]) == Sudoku.getCol(only2Indices[j][otherIndex]) ||
                            ! lines && Sudoku.getLine(only2Indices[i][otherIndex]) == Sudoku.getLine(only2Indices[j][otherIndex])) {
                        // step is X-Wing -> ignore
                        continue;
                    }
                    // can something be eliminated?
                    firstUnit.setAnd(finder.getCandidates()[cand], Sudoku.buddies[only2Indices[i][otherIndex]]);
                    firstUnit.and(Sudoku.buddies[only2Indices[j][otherIndex]]);
        ...
        for (int cand = 1; cand <= 9; cand++) {
            ...
            for (int i = 0; i < constr1Count; i++) {
                for (int j = constr1Count; j < constr1Count + constr2Count; j++) {
                    ...
                    // in only2indices[][1]
                    if (Sudoku.getBlock(only2Indices[i][0]) == Sudoku.getBlock(only2Indices[j][0])) {
                        // everything is as it should be -> do nothing
                    } else if (Sudoku.getBlock(only2Indices[i][0]) == Sudoku.getBlock(only2Indices[j][1])) {
                        int tmp = only2Indices[j][0];
                        only2Indices[j][0] = only2Indices[j][1];
                        only2Indices[j][1] = tmp;
                    } else if (Sudoku.getBlock(only2Indices[i][1]) == Sudoku.getBlock(only2Indices[j][0])) {
                        int tmp = only2Indices[i][0];
                        only2Indices[i][0] = only2Indices[i][1];
                        only2Indices[i][1] = tmp;
                    } else if (Sudoku.getBlock(only2Indices[i][1]) == Sudoku.getBlock(only2Indices[j][1])) {
                        int tmp = only2Indices[j][0];
                        only2Indices[j][0] = only2Indices[j][1];
                        only2Indices[j][1] = tmp;
                    ...
                    int crossIndex = Sudoku.getIndex(Sudoku.getLine(only2Indices[j][1]), Sudoku.getCol(only2Indices[i][1]));
