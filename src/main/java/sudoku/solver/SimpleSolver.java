...
public class SimpleSolver extends AbstractSolver {
    ...
    private boolean[] singleFound = new boolean[Sudoku.LENGTH];
    ...
    private boolean[] sameConstraint = new boolean[Sudoku.CONSTRAINTS[0].length];
    /** Buffer for checking for locked Subsets. */
    private boolean[] foundConstraint = new boolean[Sudoku.CONSTRAINTS[0].length];
    /** Buffer for checking for locked Subsets. */
    private int[] constraint = new int[Sudoku.CONSTRAINTS[0].length];
    ...
    private SolutionStep findFullHouse(boolean all) {
        ...
        while (queueIndex != -1) {
            ...
            if (sudoku.getValue(index) == 0) {
                ...
                for (int i = 0; i < Sudoku.CONSTRAINTS[index].length; i++) {
                    int constr = Sudoku.CONSTRAINTS[index][i];
...
//                        step.setEntity(Sudoku.CONSTRAINT_TYPE_FROM_CONSTRAINT[constr]);
//                        step.setEntityNumber(Sudoku.CONSTRAINT_NUMBER_FROM_CONSTRAINT[constr]);
                        step.addValue(value);
                        step.addIndex(index);
                        if (all) {
                            steps.add(step);
    ...
    private SolutionStep findNakedXle(int anz, boolean lockedOnly) {
        ...
        SolutionStep step = findNakedXleInEntity(Sudoku.BLOCKS, anz, lockedOnly, ! lockedOnly, true);
        if (step != null || lockedOnly) {
            return step;
        }
        step = findNakedXleInEntity(Sudoku.LINES, anz, lockedOnly, ! lockedOnly, true);
        if (step != null) {
            return step;
        }
        step = findNakedXleInEntity(Sudoku.COLS, anz, lockedOnly, ! lockedOnly, true);
        return step;
    }
    ...
    protected List<SolutionStep> findAllNakedXle() {
        ...
        // now everything else
        for (int i = 2; i <= 4; i++) {
            findNakedXleInEntity(Sudoku.BLOCKS, i, false, false, false);
            findNakedXleInEntity(Sudoku.LINES, i, false, false, false);
            findNakedXleInEntity(Sudoku.COLS, i, false, false, false);
        }
        Collections.sort(steps);
        steps = oldList;
        return newList;
    ...
    private SolutionStep findNakedXleInEntity(int[][] indices, int anz, 
        ...
        for (int entity = 0; entity < indices.length; entity++) {
            ...
            for (int i = 0; i < indices[entity].length; i++) {
                int tmpAnz = Sudoku.ANZ_VALUES[sudoku.getCell(indices[entity][i])];
            ...
            for (int i1 = 0; i1 < maxIndex - anz + 1; i1++) {
                ...
                for (int i2 = i1 + 1; i2 < maxIndex - anz + 2; i2++) {
                    ...
                    if (Sudoku.ANZ_VALUES[cell2] > anz) {
                        continue;
                    }
                    if (anz == 2) {
                        if (Sudoku.ANZ_VALUES[cell2] == anz) {
                        ...
                        for (int i3 = i2 + 1; i3 < maxIndex - anz + 3; i3++) {
                            ...
                            // take cell only if the combination of candidates
                            // with the current result doesnt exceed anz
                            if (Sudoku.ANZ_VALUES[cell3] > anz) {
                                continue;
                            }
                            if (anz == 3) {
                                if (Sudoku.ANZ_VALUES[cell3] == anz) {
                                ...
                                for (int i4 = i3 + 1; i4 < maxIndex; i4++) {
                                    ...
                                    // take cell only if the combination of candidates
                                    // with the current result doesnt exceed anz
                                    if (Sudoku.ANZ_VALUES[cell4] > anz) {
                                        continue;
                                    }
                                    if (Sudoku.ANZ_VALUES[cell4] == anz) {
                                        // step found!
    ...
    private SolutionStep findHiddenSingle() {
        ...
        while ((queueIndex = hsQueue.getSingle()) != -1) {
            ...
            if (sudoku.getValue(index) == 0) {
                ...
                for (int i = 0; i < Sudoku.CONSTRAINTS[index].length; i++) {
                    if (free[Sudoku.CONSTRAINTS[index][i]][value] == 1) {
    ...
    protected List<SolutionStep> findAllHiddenXle() {
        ...
        steps.addAll(tmpSteps);
        for (int i = 2; i <= 4; i++) {
            findHiddenXleInEntity(2 * Sudoku.UNITS, Sudoku.BLOCKS, i, false);
            findHiddenXleInEntity(0, Sudoku.LINES, i, false);
            findHiddenXleInEntity(Sudoku.UNITS, Sudoku.COLS, i, false);
        }
        Collections.sort(steps);
        steps = oldList;
        return newList;
    ...
    private SolutionStep findHiddenXle(int anz) {
        SudokuUtil.clearStepList(steps);
        SolutionStep step = findHiddenXleInEntity(2 * Sudoku.UNITS, Sudoku.BLOCKS, anz, true);
        if (step != null) {
            return step;
        }
        step = findHiddenXleInEntity(0, Sudoku.LINES, anz, true);
        if (step != null) {
            return step;
        }
        step = findHiddenXleInEntity(Sudoku.UNITS, Sudoku.COLS, anz, true);
        return step;
    }
        ...
        for (int entity = 0; entity < indices.length; entity++) {
            ...
            for (int i = 1; i <= 9; i++) {
                ...
                if (actFree != 0 && actFree <= anz) {
                    ...
                    candMask |= Sudoku.MASKS[i];
                    ipcMask[i] = 0;
                    for (int j = 0; j < Sudoku.UNITS; j++) {
                        if ((sudoku.getCell(indices[entity][j]) & Sudoku.MASKS[i]) != 0) {
                            ipcMask[i] |= Sudoku.MASKS[j + 1];
                        }
                    }
                }
            }
            if (Sudoku.ANZ_VALUES[candMask] < anz) {
                // not enough candidates
                continue;
            }
            // now check all possible combinations of candidates
            int[] candArr = Sudoku.POSSIBLE_VALUES[candMask];
            // first level: take all candidates
            for (int i1 = 0; i1 < candArr.length - anz + 1; i1++) {
                short cand1 = Sudoku.MASKS[candArr[i1]];
                short cell1 = ipcMask[candArr[i1]];
                // second level: always necessary.
                for (int i2 = i1 + 1; i2 < candArr.length - anz + 2; i2++) {
                    short cand2 = (short)(cand1 | Sudoku.MASKS[candArr[i2]]);
                    short cell2 = (short)(cell1 | ipcMask[candArr[i2]]);
                    if (anz == 2) {
                        if (Sudoku.ANZ_VALUES[cell2] == anz) {
                            // step found!
                            int[] tmp = Sudoku.POSSIBLE_VALUES[cell2];
                        ...
                        for (int i3 = i2 + 1; i3 < candArr.length - anz + 3; i3++) {
                            short cand3 = (short)(cand2 | Sudoku.MASKS[candArr[i3]]);
                            short cell3 = (short)(cell2 | ipcMask[candArr[i3]]);
                            if (anz == 3) {
                                if (Sudoku.ANZ_VALUES[cell3] == anz) {
                                    // step found!
                                    int[] tmp = Sudoku.POSSIBLE_VALUES[cell3];
                                    step = createSubsetStep(indices[entity][tmp[0] - 1], indices[entity][tmp[1] - 1],
                                ...
                                for (int i4 = i3 + 1; i4 < candArr.length; i4++) {
                                    short cand4 = (short)(cand3 | Sudoku.MASKS[candArr[i4]]);
                                    short cell4 = (short)(cell3 | ipcMask[candArr[i4]]);
                                    if (Sudoku.ANZ_VALUES[cell4] == anz) {
                                        // step found!
                                        int[] tmp = Sudoku.POSSIBLE_VALUES[cell4];
                                        step = createSubsetStep(indices[entity][tmp[0] - 1], indices[entity][tmp[1] - 1],
    ...
    private SolutionStep findLockedCandidates(SolutionType type) {
        ...
        if (type == SolutionType.LOCKED_CANDIDATES || type == SolutionType.LOCKED_CANDIDATES_1) {
            step = findLockedCandidatesInEntityN(18, Sudoku.BLOCKS, true);
            if (step != null) {
                return step;
            }
        }
        if (type == SolutionType.LOCKED_CANDIDATES || type == SolutionType.LOCKED_CANDIDATES_2) {
            step = findLockedCandidatesInEntityN(0, Sudoku.LINES, true);
            if (step != null) {
                return step;
            }
            step = findLockedCandidatesInEntityN(9, Sudoku.COLS, true);
            if (step != null) {
                return step;
            }
        }
    ...
    protected List<SolutionStep> findAllLockedCandidates() {
        ...
        List<SolutionStep> newList = new ArrayList<SolutionStep>();
        steps = newList;
        findLockedCandidatesInEntityN(18, Sudoku.BLOCKS, false);
        findLockedCandidatesInEntityN(0, Sudoku.LINES, false);
        findLockedCandidatesInEntityN(9, Sudoku.COLS, false);
        Collections.sort(steps);
        steps = oldList;
        return newList;
    }
    ...
    private SolutionStep findLockedCandidatesInEntityN(int constraintBase, int[][] indices, boolean onlyOne) {
        ...
        SolutionStep step = null;
        byte[][] free = sudoku.getFree();
        for (int constr = 0; constr < Sudoku.UNITS; constr++) {
            ...
            for (int cand = 1; cand <= 9; cand++) {
                ...
                if (unitFree == 2 || unitFree == 3) {
                    ...
                    for (int i = 0; i < indices[constr].length; i++) {
                        ...
                        if ((cell & Sudoku.MASKS[cand]) == 0) {
                            // candidate not present -> skip the cell
                            continue;
                        }
                        if (first) {
                            constraint[0] = Sudoku.CONSTRAINTS[index][0];
                            constraint[1] = Sudoku.CONSTRAINTS[index][1];
                            constraint[2] = Sudoku.CONSTRAINTS[index][2];
                            first = false;
                        } else {
                            for (int j = 0; j < Sudoku.CONSTRAINTS[0].length; j++) {
                                if (sameConstraint[j] && constraint[j] != Sudoku.CONSTRAINTS[index][j]) {
                    ...
                    if (constraintBase == 18) {
                        ...
                        step = createLockedCandidatesStep(SolutionType.LOCKED_CANDIDATES_1, cand,
                                skipConstraint, Sudoku.ALL_UNITS[aktConstraint]);
                        if (onlyOne) {
                            return step;
                        } else {
                            steps.add(step);
                        ...
                        if (sameConstraint[2] && free[constraint[2]][cand] > unitFree) {
                            ...
                            step = createLockedCandidatesStep(SolutionType.LOCKED_CANDIDATES_2, cand,
                                    skipConstraint, Sudoku.ALL_UNITS[constraint[2]]);
                            if (onlyOne) {
                                return step;
                            } else {
                                steps.add(step);
    ...
    private SolutionStep createLockedCandidatesStep(SolutionType type, int cand, int skipConstraint, int[] indices) {
        ...
        globalStep.setEntity(Sudoku.CONSTRAINT_TYPE_FROM_CONSTRAINT[skipConstraint]);
        globalStep.setEntityNumber(Sudoku.CONSTRAINT_NUMBER_FROM_CONSTRAINT[skipConstraint]);
        // check all cells in the unit
        for (int i = 0; i < indices.length; i++) {
            int index = indices[i];
            if ((sudoku.getCell(index) & Sudoku.MASKS[cand]) != 0) {
                // cell holds the candidate
                if (Sudoku.CONSTRAINTS[index][0] == skipConstraint ||
                        Sudoku.CONSTRAINTS[index][1] == skipConstraint ||
                        Sudoku.CONSTRAINTS[index][2] == skipConstraint) {
...
//        System.out.println("create SubsetStep: " + type + " (" + Arrays.toString(indices) + ")");
        ...
        sameConstraint[0] = sameConstraint[1] = sameConstraint[2] = true;
        constraint[0] = Sudoku.CONSTRAINTS[indices[0]][0];
        constraint[1] = Sudoku.CONSTRAINTS[indices[0]][1];
        constraint[2] = Sudoku.CONSTRAINTS[indices[0]][2];
        for (int i = 1; i < indices.length; i++) {
            for (int j = 0; j < Sudoku.CONSTRAINTS[0].length; j++) {
                if (sameConstraint[j] && constraint[j] != Sudoku.CONSTRAINTS[indices[i]][j]) {
                    sameConstraint[j] = false;
                }
            }
        }
...
//        System.out.println("constraint: " + Arrays.toString(constraint));
        ...
        if (type.isHiddenSubset()) {
            ...
            for (int i = 0; i < indices.length; i++) {
                ...
                int[] candArray = Sudoku.POSSIBLE_VALUES[candsToDelete];
            ...
            for (int i = 0; i < sameConstraint.length; i++) {
                ...
                int[] cells = Sudoku.ALL_UNITS[constraint[i]];
...
//                    System.out.println("  candArray: " + Arrays.toString(candArray));
                    for (int k = 0; k < candArray.length; k++) {
                        ...
                        if (! foundConstraint[i] && ((i == 2) ||
                           (Sudoku.CONSTRAINTS[cells[j]][2] != constraint[2]))) {
        ...
        int[] candArray = Sudoku.POSSIBLE_VALUES[cands];
