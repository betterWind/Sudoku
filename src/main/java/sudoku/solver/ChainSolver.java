...
public class ChainSolver extends AbstractSolver {
    ...
    private static final int MAX_CHAIN_LENGTH = 2 * Sudoku.LENGTH;
...
//                        }
                    ...
                    startCellSetM1 = Sudoku.buddiesM1[startIndex] & finder.getCandidates()[startCandidate].getMask1();
                    startCellSetM2 = Sudoku.buddiesM2[startIndex] & finder.getCandidates()[startCandidate].getMask2();
                    if (type == REMOTE_PAIR) {
                        // the same for the second candidate in a RP
                        startCellSet2M1 = Sudoku.buddiesM1[startIndex] & finder.getCandidates()[startCandidate2].getMask1();
                        startCellSet2M2 = Sudoku.buddiesM2[startIndex] & finder.getCandidates()[startCandidate2].getMask2();
                    }
                    // now do it
                    getChain(entry, type);
                }
...
//                chainSet.remove(Chain.getSCellIndex(chain[stackLevel]));
            ...
            if (typ == NICE_LOOP) {
                ...
                if (stackLevel > 1 && newLinkIsStrong && newLinkCandidate == startCandidate) {
                    ...
                    long m1 = startCellSetM1 & Sudoku.buddiesM1[newLinkIndex];
                    long m2 = startCellSetM2 & Sudoku.buddiesM2[newLinkIndex];
        ...
        if (stackLevel > 7) {
            ...
            for (int i = 0; i <= stackLevel; i += 2) {
                ...
                for (int j = i + 6; j <= stackLevel; j += 4) {
                    rpTmp.set(Sudoku.buddies[Chain.getSCellIndex(chain[i])]);
                    rpTmp.and(Sudoku.buddies[Chain.getSCellIndex(chain[j])]);
            ...
            long m21 = startCellSet2M1 & Sudoku.buddiesM1[endIndex];
            long m22 = startCellSet2M2 & Sudoku.buddiesM2[endIndex];
            rpCand1.set(m1, m2);
            rpCand2.set(m21, m22);
        }
        // no create the step
...
//            }
    ...
    private void getAllLinks(int type) {
        ...
        for (int cellIndex = 0; cellIndex < sudoku.getCells().length; cellIndex++) {
            ...
            if (cell == 0 || ((type == REMOTE_PAIR || type == XY_CHAIN) && Sudoku.ANZ_VALUES[cell] != 2)) {
                // ignore filled cells
                continue;
            }
...
//                if (sudoku.getValue(cellIndex) != 0 || !sudoku.isCandidate(cellIndex, cellCandidate)) {
                ...
                int[] cands = Sudoku.POSSIBLE_VALUES[cell];
                if (Sudoku.ANZ_VALUES[cell] == 2) {
                    // bivalue cell
                    if (type != X_CHAIN && type != TURBOT_FISH) {
                        int cand = cands[0];
...
//                        System.out.println("  " + index + ": " + cellIndex + "/" + cands[k] + "/" + false);
                ...
                for (int constr = 0; constr < Sudoku.CONSTRAINTS[cellIndex].length; constr++) {
                    boolean strong = false;
                    if (free[Sudoku.CONSTRAINTS[cellIndex][constr]][cellCandidate] == 2) {
                        // strong link!
                        if (type == X_CHAIN || type == TURBOT_FISH || type == NICE_LOOP) {
                            strong = true;
                        }
                    }
                    int[] indices = Sudoku.ALL_UNITS[Sudoku.CONSTRAINTS[cellIndex][constr]];
                    for (int k = 0; k < indices.length; k++) {
                        if (indices[k] != cellIndex && sudoku.isCandidate(indices[k], cellCandidate)) {
                            ...
                            if (type == XY_CHAIN && Sudoku.ANZ_VALUES[cell2] != 2) {
                                // must be bivalue
                                continue;
                            }
                            if (constr == 2 && (Sudoku.getLine(cellIndex) == Sudoku.getLine(indices[k]) ||
                                    Sudoku.getCol(cellIndex) == Sudoku.getCol(indices[k]))) {
