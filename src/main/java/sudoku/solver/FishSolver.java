...
public class FishSolver extends AbstractSolver {
    ...
    private int maxSize;
    /**
     * An array with all possible base units (indices in {@link Sudoku#CONSTRAINTS}).
     */
    private int[] baseUnits = new int[Sudoku.UNITS * 3];
    /**
     * For every base unit all cells where the candiate is set (low order DWORD).
     */
    private long[] baseCandidatesM1 = new long[Sudoku.UNITS * 3];
    /**
     * For every base unit all cells where the candiate is set (high order DWORD).
     */
    private long[] baseCandidatesM2 = new long[Sudoku.UNITS * 3];
    /**
     * The number of base units in this search
     */
    private int numberOfBaseUnits = 0;
    /**
     * An array with all possible cover units (indices in {@link Sudoku#CONSTRAINTS}).
     */
    private int[] allCoverUnits = new int[Sudoku.UNITS * 3];
    /**
     * For every cover unit all cells where the candiate is set (low order DWORD).
     */
    private long[] allCoverCandidatesM1 = new long[Sudoku.UNITS * 3];
    /**
     * For every cover unit all cells where the candiate is set (high order DWORD).
     */
    private long[] allCoverCandidatesM2 = new long[Sudoku.UNITS * 3];
    /**
     * The number of possible cover units
     */
    private int numberOfAllCoverUnits = 0;
    /**
     * An array with all possible cover units for one cover unit search (indices in {@link Sudoku#CONSTRAINTS}).
     */
    private int[] coverUnits = new int[Sudoku.UNITS * 3];
    /**
     * For every entry in {@link #coverUnits} all cells where the candiate is set (low order DWORD).
     */
    private long[] coverCandidatesM1 = new long[Sudoku.UNITS * 3];
    /**
     * For every entry in {@link #coverUnits} all cells where the candiate is set (high order DWORD).
     */
    private long[] coverCandidatesM2 = new long[Sudoku.UNITS * 3];
    /**
...
//    protected List<SolutionStep> getAllFishes(int minSize, int maxSize, int maxFins, int maxEndoFins, FindAllStepsProgressDialog dlg, int forCandidate, int type) {
    protected List<SolutionStep> getAllFishes(int minSize, int maxSize, int maxFins, int maxEndoFins, int forCandidate, int type) {
...
//        this.dlg = null;
     ...
     */
    // TODO: 2019-07-06
//    protected List<SolutionStep> getAllKrakenFishes(int minSize, int maxSize, int maxFins, int maxEndoFins, FindAllStepsProgressDialog dlg, int forCandidate, int type) {
    protected List<SolutionStep> getAllKrakenFishes(int minSize, int maxSize, int maxFins, int maxEndoFins, int forCandidate, int type) {
...
//            if (doTemplates && ! entry.endoFins.isEmpty()) {
            if (doTemplates && (bEntry.endoFinsM1 != 0 || bEntry.endoFinsM2 != 0)) {
                ...
                Sudoku.getBuddies(bEntry.endoFinsM1, bEntry.endoFinsM2, getBuddiesSet);
...
//                    Sudoku.getBuddies(fins, finBuddies);
                    Sudoku.getBuddies(finsM1, finsM2, getBuddiesSet);
...
//                    checkSashimiSet.andNot(finSet);
        ...
        for (int i = 0; i < baseUnitsUsed.length; i++) {
            if (baseUnitsUsed[i]) {
                globalStep.addBaseEntity(Sudoku.CONSTRAINT_TYPE_FROM_CONSTRAINT[i],
                        Sudoku.CONSTRAINT_NUMBER_FROM_CONSTRAINT[i]);
            }
        }
        for (int i = 0; i < coverUnitsUsed.length; i++) {
            if (coverUnitsUsed[i]) {
                globalStep.addCoverEntity(Sudoku.CONSTRAINT_TYPE_FROM_CONSTRAINT[i],
                        Sudoku.CONSTRAINT_NUMBER_FROM_CONSTRAINT[i]);
            }
        }
    ...
    private int getUnitMask(boolean[] used) {
        ...
        for (int i = 0; i < used.length; i++) {
            if (used[i]) {
                mask |= MASKS[Sudoku.CONSTRAINT_TYPE_FROM_CONSTRAINT[i]];
            }
        }
        return mask;
    }
    ...
    private void initForCandidat(int size, boolean withFins, boolean lines) {
        ...
        // go through all sets
        for (int i = 0; i < Sudoku.ALL_CONSTRAINTS_TEMPLATES.length; i++) {
            if (i >= 18 && fishType == BASIC) {
                continue;
            }
//            tmpSet.set(Sudoku.ALL_CONSTRAINTS_TEMPLATES[i]);
//            tmpSet.and(candidates);
            tmpSetM1 = Sudoku.ALL_CONSTRAINTS_TEMPLATES_M1[i] & candidatesM1;
            tmpSetM2 = Sudoku.ALL_CONSTRAINTS_TEMPLATES_M2[i] & candidatesM2;
//            if (tmpSet.isEmpty()) {
