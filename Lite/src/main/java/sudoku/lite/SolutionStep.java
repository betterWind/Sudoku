...
import sudoku.lite.solver.Als;
import sudoku.lite.solver.RestrictedCommon;
...
//        return getForcingChainString(chain.getChain(), chain.getStart(), chain.getEnd(), false);
    ...
    public String getCandidateString(boolean library, boolean statistics) {
        ...
        for (Candidate cand : candidatesToDelete) {
            ...
            delPos.append(" ");
            if (library) {
                delPos.append(Integer.toString(cand.getValue())).append(Integer.toString(Sudoku.getLine(cand.getIndex()) + 1)).
                        append(Integer.toString(Sudoku.getCol(cand.getIndex()) + 1));
            }
        }
        if (library) {
...
//                    }
    ...
    public static String getAls(Als als, boolean withCandidates) {
        ...
        if (withCandidates) {
            //tmp.append(" - {");
            tmp.append(" {");
            int[] cands = Sudoku.POSSIBLE_VALUES[als.candidates];
...
//        }
    ...
    public void setEntity(int entity) {
        if (entity != Sudoku.BLOCK && entity != Sudoku.LINE && entity != Sudoku.COL && entity != Sudoku.CELL) {
...
//    }
    ...
    public void addAls(SudokuSet indices, short candidates) {
        ...
        int[] cands = Sudoku.POSSIBLE_VALUES[candidates];
        for (int i = 0; i < cands.length; i++) {
