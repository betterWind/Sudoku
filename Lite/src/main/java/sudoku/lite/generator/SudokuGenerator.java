...
public class SudokuGenerator {
    ...
    private static Sudoku EMPTY_GRID = new Sudoku();
    ...
    public void solve(String sudokuString) {
        ...
        // set up the sudoku
        for (int i = 0; i < sudokuString.length() && i < Sudoku.LENGTH; i++) {
...
//        System.out.println("unsolvedCellsAnz = " + stack[0].sudoku.getUnsolvedCellsAnz());
        if (stack[0].sudoku.getUnsolvedCellsAnz() == 0) {
            ...
            solution = Arrays.copyOf(stack[0].sudoku.getValues(), Sudoku.LENGTH);
            solutionCount++;
            if (DEBUG) {
        ...
        while (true) {
            ...
            if (stack[level].sudoku.getUnsolvedCellsAnz() == 0) {
                ...
                if (solutionCount == 1) {
                    ...
                    solution = Arrays.copyOf(stack[level].sudoku.getValues(), Sudoku.LENGTH);
                ...
                int index = -1;
                int anzCand = 9;
                Sudoku sudoku = stack[level].sudoku;
                for (int i = 0; i < Sudoku.LENGTH; i++) {
//                    if (sudoku.getCell(i) != 0) {
//                        System.out.println("cell[" + i + "] = " + Sudoku.ANZ_VALUES[sudoku.getCell(i)]);
//                    }
                    if (sudoku.getCell(i) != 0 && Sudoku.ANZ_VALUES[sudoku.getCell(i)] < anzCand) {
                        index = i;
                        anzCand = Sudoku.ANZ_VALUES[sudoku.getCell(i)];
                    }
                }
                level++;
                ...
                stack[level].candidates = Sudoku.POSSIBLE_VALUES[stack[level - 1].sudoku.getCell(index)];
     ...
     * @return
     */
    public Sudoku generateSudoku(boolean symmetric) {
     ...
     * @return 
     */
    public Sudoku generateSudoku(boolean symmetric, boolean[] pattern) {
        generateFullGrid();
        if (pattern == null) {
...
//            nanos += System.nanoTime() - actNanos;
     ...
     */
    private boolean setAllExposedSingles(Sudoku sudoku) {
        boolean valid = true;
        ...
        do {
            ...
            while (valid && (singleIndex = nsQueue.getSingle()) != -1) {
                ...
                if ((sudoku.getCell(index) & Sudoku.MASKS[value]) != 0) {
    ...
    private String getGridStr(Sudoku sudoku) {
...
//        64.7............53.......1.7.86........4.9...5.........6....4......5.2......1....
        ...
        sudokuGenerator.solve("..15............32...............2.9.5...3......7..8..27.....4.3...9.......6..5..");
        System.out.println(sudokuGenerator.getSolutionAsString() + " (" + sudokuGenerator.getSolutionCount() + ")");
        sudokuGenerator.solve(".1.....2....8..6.......3........43....2.1....8......9.4...7.5.3...2...........4..");
        System.out.println(sudokuGenerator.getSolutionAsString() + " (" + sudokuGenerator.getSolutionCount() + ")");
        sudokuGenerator.solve("...87..3.52.......4..........3.9..7......54...8.......2.....5.....3....9...1.....");
        System.out.println(sudokuGenerator.getSolutionAsString() + " (" + sudokuGenerator.getSolutionCount() + ")");
        sudokuGenerator.solve(".51..........2.4...........64....2.....5.1..7...3..6..4...3.......8...5.2........");
        System.out.println(sudokuGenerator.getSolutionAsString() + " (" + sudokuGenerator.getSolutionCount() + ")");
        sudokuGenerator.solve("17.....4....62....5...3....84....1.....3....6......9....6.....3.....1..........5.");
        System.out.println(sudokuGenerator.getSolutionAsString() + " (" + sudokuGenerator.getSolutionCount() + ")");
        sudokuGenerator.solve("...7...1...6.......4.......7..5.1.....8...4..2...........24.6...3..8....1.......9");
        System.out.println(sudokuGenerator.getSolutionAsString() + " (" + sudokuGenerator.getSolutionCount() + ")");
        sudokuGenerator.solve("3.....7.....1..4.....2.........5.61..82...........6....1.....287...3...........3.");
        System.out.println(sudokuGenerator.getSolutionAsString() + " (" + sudokuGenerator.getSolutionCount() + ")");
        sudokuGenerator.solve("64.7............53.......1.7.86........4.9...5.........6....4......5.2......1....");
        System.out.println(sudokuGenerator.getSolutionAsString() + " (" + sudokuGenerator.getSolutionCount() + ")");
        sudokuGenerator.solve("1..2..3...2.1.........3...1....1.2.3..1...4....2.4...........1.2.........1.......");
        System.out.println(sudokuGenerator.getSolutionAsString() + " (" + sudokuGenerator.getSolutionCount() + ")");
        ...
        for (int i = 0; i < anzRuns; i++) {
            sudokuGenerator.solve("..15............32...............2.9.5...3......7..8..27.....4.3...9.......6..5..");
            sudokuGenerator.solve(".1.756.........2..4...9.57...95...21.........76...14...91.7...3..7.........635.9.");
//              2 Easter Monster
            sudokuGenerator.solve("1.......2.9.4...5...6...7...5.9.3.......7.......85..4.7.....6...3...9.8...2.....1");
//              2
            sudokuGenerator.solve("..............3.85..1.2.......5.7.....4...1...9.......5......73..2.1........4...9");
//             95
            sudokuGenerator.solve(".1.....2....8..6.......3........43....2.1....8......9.4...7.5.3...2...........4..");
//              1
            sudokuGenerator.solve("...87..3.52.......4..........3.9..7......54...8.......2.....5.....3....9...1.....");
//             73
            sudokuGenerator.solve("..15............32...............2.9.5...3......7..8..27.....4.3...9.......6..5..");
//              0
            sudokuGenerator.solve("7.....4...2..7..8...3..8..9...5..3...6..2..9...1..7..6...3..9...3..4..6...9..1..5");
//              1 sudokuGenerator.solve("...7..8......4..3......9..16..5......1..3..4...5..1..75..2..6...3..8..9...7.....2");
            sudokuGenerator.solve("536020900008000000000000000600285009000903000800761004000000000004000000201000007");
            sudokuGenerator.solve("000000490020100000000000500800400300609000000000200000000069070040050000000000001");
            sudokuGenerator.solve(".1.....2....8..6.......3........43....2.1....8......9.4...7.5.3...2...........4..");
            sudokuGenerator.solve("...87..3.52.......4..........3.9..7......54...8.......2.....5.....3....9...1.....");
            sudokuGenerator.solve("..1..4.......6.3.5...9.....8.....7.3.......285...7.6..3...8...6..92......4...1...");
            sudokuGenerator.solve("7..4......5....3........1..368..........2..5....7........5...7213...8...6........");
        }
        ticks = System.currentTimeMillis() - ticks;
        System.out.println(sudokuGenerator.getSolutionAsString() + " (" + sudokuGenerator.getSolutionCount() + ")");
        System.out.println("Time: " + ((double) ticks / anzRuns) + "ms");
        System.out.println(sudokuGenerator.printStat());

        ticks = System.currentTimeMillis();
        for (int i = 0; i < anzRuns; i++) {
            Sudoku sudoku = sudokuGenerator.generateSudoku(true);
            sudokuGenerator.generateFullGrid();
            System.out.println("New Grid1: " + sudokuGenerator.getSolutionAsString(sudokuGenerator.newFullSudoku));
            System.out.println("New Grid2: " + sudokuGenerator.getSolutionAsString(sudokuGenerator.newValidSudoku));
        ...
        ticks = System.currentTimeMillis() - ticks;
        //System.out.println("New Grid: " + sudokuGenerator.getSolutionAsString(sudokuGenerator.newFullSudoku));
        long faktor = 1000000;
        long nanos = sudokuGenerator.nanos / faktor;
        long setNanos = sudokuGenerator.setNanos / faktor;
        System.out.println("Time: " + ((double) ticks / anzRuns) + "ms " + (sudokuGenerator.anzTriesGen / anzRuns) + "/" + (sudokuGenerator.anzClues / anzRuns) + "/" +
