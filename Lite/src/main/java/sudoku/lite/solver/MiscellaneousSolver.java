...
public class MiscellaneousSolver extends AbstractSolver {
    ...
    private StackEntry[] stack1 = new StackEntry[Sudoku.UNITS];
    /** Stack for searching blocks */
    private StackEntry[] stack2 = new StackEntry[Sudoku.UNITS];
    ...
    protected List<SolutionStep> getAllSueDeCoqs() {
        ...
        getSueDeCoqInt(Sudoku.LINE_TEMPLATES, Sudoku.BLOCK_TEMPLATES, false);
        getSueDeCoqInt(Sudoku.COL_TEMPLATES, Sudoku.BLOCK_TEMPLATES, false);
    ...
    private SolutionStep getSueDeCoq(boolean onlyOne) {
        SolutionStep step = getSueDeCoqInt(Sudoku.LINE_TEMPLATES, Sudoku.BLOCK_TEMPLATES, onlyOne);
        if (onlyOne && step != null) {
            return step;
        }
        return getSueDeCoqInt(Sudoku.COL_TEMPLATES, Sudoku.BLOCK_TEMPLATES, onlyOne);
    }
    ...
    private SolutionStep checkIntersection(boolean onlyOne) {
        ...
        for (int i1 = 0; i1 < max - 1; i1++) {
            ...
            for (int i2 = i1 + 1; i2 < max; i2++) {
                ...
                nPlus = Sudoku.ANZ_VALUES[cand2] - 2;
                if (nPlus >= 2) {
                    // possible SDC -> check
                ...
                for (int i3 = i2 + 1; i3 < max; i3++) {
                    ...
                    nPlus = Sudoku.ANZ_VALUES[cand3] - 3;
                    if (nPlus >= 2) {
                        // possible SDC -> check
    ...
    private SolutionStep checkHouses(int nPlus, short cand, boolean onlyOne) {
        ...
        return checkHouses(nPlus, nonBlockSourceSet, Sudoku.MAX_MASK, onlyOne, false);
    }
    ...
    private SolutionStep checkHouses(int nPlus, SudokuSet sourceSet,
        ...
        while (true) {
            ...
            if ((stack[level].candidates & ~allowedCandSet) == 0) {
                ...
                int anzContained = Sudoku.ANZ_VALUES[tmpCands]; // number of candidates from the intersection
                tmpCands = (short) (stack[level].candidates & ~intersectionActCandSet);
                int anzExtra = Sudoku.ANZ_VALUES[tmpCands]; // number of candidates not drawn from the intersection
...
//                        tmpSet.andNot(blockActSet); // ???
                        ...
                        if (globalStep.getCandidatesToDelete().size() > 0) {
                            ...
                            int[] cands = Sudoku.POSSIBLE_VALUES[intersectionActCandSet];
    ...
    private void getSetCandidates(SudokuSet srcSet1, SudokuSet srcSet2, short candSet, List<Candidate> dest) {
        ...
        for (int i = 0; i < tmpSet.size(); i++) {
            ...
            if ((sudoku.getCell(index) & candSet) != 0) {
                int[] cands = Sudoku.POSSIBLE_VALUES[sudoku.getCell(index) & candSet];
                for (int j = 0; j < cands.length; j++) {
    ...
    private void checkCandidatesToDelete(SudokuSet tmpSet, short tmpCandSet) {
        ...
        if (tmpSet.size() > 0 && Sudoku.ANZ_VALUES[tmpCandSet] > 0) {
            for (int i = 0; i < tmpSet.size(); i++) {
                int index = tmpSet.get(i);
                ...
                int[] cands = Sudoku.POSSIBLE_VALUES[elimCandMask];
                for (int j = 0; j < cands.length; j++) {
