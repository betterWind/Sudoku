...
//        }
    ...
    private void checkForcingChains() {
        ...
        checkAllChainsForHouse(Sudoku.LINE_TEMPLATES);
        checkAllChainsForHouse(Sudoku.COL_TEMPLATES);
        checkAllChainsForHouse(Sudoku.BLOCK_TEMPLATES);
    }
...
//        System.out.println("replaceorcopystep: " + globalStep.toString(2));
    ...
    private void checkOneChain(TableEntry entry) {
        ...
        if ((entry.isStrong(0) && entry.offSets[entry.getCandidate(0)].contains(entry.getCellIndex(0)))
            ...
            globalStep.setEntity(Sudoku.CELL);
        ...
        checkHouseSet(entry, Sudoku.LINE_TEMPLATES, Sudoku.LINE);
        checkHouseSet(entry, Sudoku.COL_TEMPLATES, Sudoku.COL);
        checkHouseSet(entry, Sudoku.BLOCK_TEMPLATES, Sudoku.BLOCK);
        ...
        checkHouseDel(entry, Sudoku.LINE_TEMPLATES, Sudoku.LINE);
        checkHouseDel(entry, Sudoku.COL_TEMPLATES, Sudoku.COL);
        checkHouseDel(entry, Sudoku.BLOCK_TEMPLATES, Sudoku.BLOCK);
    }
...
//                        if (sudoku.getAnzCandidates(i) == 2) {
                    ...
                    tmpSet1.remove(i);
                    for (int constrIndex = 0; constrIndex < Sudoku.CONSTRAINTS[i].length; constrIndex++) {
                        // number of candidates remaining in the current house: 1 - only
                        // cand itself; 2 - strong link; > 2 - weak links
                        int constr = Sudoku.CONSTRAINTS[i][constrIndex];
                        ...
                        // get the candidates
                        tmpSet.set(tmpSet1);
                        tmpSet.and(Sudoku.ALL_CONSTRAINTS_TEMPLATES[constr]);
                        if (tmpSet.isEmpty()) {
                            // no candidates left...
                            continue;
                        }
            ...
            // one copy is enough, Sudoku.set() copies the contents of the sudoku
    ...
    private void fillTablesWithGroupNodes() {
        ...
        for (int i = 0; i < groupNodes.size(); i++) {
            ...
            if (!tmpSet.isEmpty()) {
                ...
                tmpSet1.set(tmpSet);
                tmpSet1.and(Sudoku.BLOCK_TEMPLATES[gn.block]);
                ...
                tmpSet1.set(tmpSet);
                if (gn.line != -1) {
                    tmpSet1.and(Sudoku.LINE_TEMPLATES[gn.line]);
                } else {
                    tmpSet1.and(Sudoku.COL_TEMPLATES[gn.col]);
                }
                if (!tmpSet1.isEmpty() && tmpSet1.size() == 1) {
                    offEntry.addEntry(tmpSet1.get(0), gn.cand, true);
                    TableEntry tmp = offTable[tmpSet1.get(0) * 10 + gn.cand];
            ...
            if (lineAnz == 1) {
                ...
                tmpSet.set(Sudoku.LINE_TEMPLATES[gn.line]);
            ...
            if (colAnz == 1) {
                ...
                tmpSet.set(Sudoku.COL_TEMPLATES[gn.col]);
                tmpSet.and(finder.getCandidates()[gn.cand]);
                tmpSet.andNot(gn.indices);
                tmpSet.andNot(gn2.indices);
                if (tmpSet.isEmpty()) {
            ...
            if (blockAnz == 1) {
                ...
                tmpSet.set(Sudoku.BLOCK_TEMPLATES[gn.block]);
                tmpSet.and(finder.getCandidates()[gn.cand]);
                tmpSet.andNot(gn.indices);
                tmpSet.andNot(gn2.indices);
                if (tmpSet.isEmpty()) {
...
//                        offEntry.addEntry(cellIndex, tmpSet1.get(0), true);
    ...
    private void setCell(int cellIndex, int cand, TableEntry entry, boolean getRetIndices, boolean nakedSingle) {
        ...
        int entityType = Sudoku.LINE;
        int entityNumberFree = sudoku.getFree()[Sudoku.CONSTRAINTS[cellIndex][0]][cand];
        int dummy = sudoku.getFree()[Sudoku.CONSTRAINTS[cellIndex][1]][cand];
        if (dummy < entityNumberFree) {
            entityType = Sudoku.COL;
            entityNumberFree = dummy;
        }
        dummy = sudoku.getFree()[Sudoku.CONSTRAINTS[cellIndex][2]][cand];
        if (dummy < entityNumberFree) {
            entityType = Sudoku.BLOCK;
            entityNumberFree = dummy;
        }
        // now set the cell
        ...
        if (getRetIndices) {
            ...
            if (nakedSingle) {
                ...
                if (entityType == Sudoku.LINE) {
                    getRetIndicesForHouse(cellIndex, cand, Sudoku.LINE_TEMPLATES[Sudoku.getLine(cellIndex)], entry);
                } else if (entityType == Sudoku.COL) {
                    getRetIndicesForHouse(cellIndex, cand, Sudoku.COL_TEMPLATES[Sudoku.getCol(cellIndex)], entry);
                } else {
                    getRetIndicesForHouse(cellIndex, cand, Sudoku.BLOCK_TEMPLATES[Sudoku.getBlock(cellIndex)], entry);
                }
            }
