...
import sudoku.lite.SolutionStep;
import sudoku.lite.Sudoku;
import sudoku.lite.SudokuSet;
...
public class Als {
    ...
    public void computeFields(SudokuStepFinder finder) {
        ...
        for (int i = 1; i <= 9; i++) {
            if ((candidates & Sudoku.MASKS[i]) != 0) {
                ...
                Sudoku.getBuddies(indicesPerCandidat[i], buddiesPerCandidat[i]);
    ...
    public int getChainPenalty() {
        if (chainPenalty == -1) {
            chainPenalty = getChainPenalty(Sudoku.ANZ_VALUES[candidates]);
        }
        return chainPenalty;
    }
