...
//                }
    ...
    private int doColoring(int cand) {
        ...
        for (int i = 0; i < size; i++) {
            int index = values[i];
            if (free[Sudoku.CONSTRAINTS[index][0]][cand] != 2 &&
                    free[Sudoku.CONSTRAINTS[index][1]][cand] != 2 &&
                    free[Sudoku.CONSTRAINTS[index][2]][cand] != 2) {
    ...
    private void doColoringForColorRecursive(int index, int cand, boolean on) {
        ...
        // recursion
        doColoringForColorRecursive(getConjugateIndex(index, cand, Sudoku.CONSTRAINTS[index][0]), cand, !on);
        doColoringForColorRecursive(getConjugateIndex(index, cand, Sudoku.CONSTRAINTS[index][1]), cand, !on);
        doColoringForColorRecursive(getConjugateIndex(index, cand, Sudoku.CONSTRAINTS[index][2]), cand, !on);
    }
    ...
    private int getConjugateIndex(int index, int cand, int constraint) {
        ...
        tmpSet1.and(Sudoku.ALL_CONSTRAINTS_TEMPLATES[constraint]);
